...
//最多的变量数为16(16个还不够你用?)
	...
	Menu[1].Title = "SYSTEM";
	Menu[1].Sub_Value[0] = &g_nSpeedExpect;
	Menu[1].Sub_Title[0] = "ASK_SPEED";
	Menu[1].Sub_Type[0] = -2;
	Menu[1].Sub_Value[1] = &Flag_timeset;
	Menu[1].Sub_Title[1] = "Times_Set";
	Menu[1].Sub_Type[1] = -2;
	
	Menu[2].Title = "PID_SPEED";
	Menu[2].Sub_Value[0] = &g_PID_Speed.P;
	Menu[2].Sub_Title[0] = "SPEED_P";
	Menu[2].Sub_Type[0] = -4;
	Menu[2].Sub_Value[1] = &g_PID_Speed.I;
	Menu[2].Sub_Title[1] = "SPEED_I";
	Menu[2].Sub_Type[1] = -4;
	Menu[2].Sub_Value[2] = &g_PID_Speed.D;
	Menu[2].Sub_Title[2] = "SPEED_D";
	Menu[2].Sub_Type[2] = -4;
	
	Menu[3].Title = "PID_DIR";
	Menu[3].Sub_Value[0] = &g_PID_DIR.P;
	Menu[3].Sub_Title[0] = "ANGER_P";
	Menu[3].Sub_Type[0] = -4;
	Menu[3].Sub_Value[1] = &g_PID_DIR.I;
	Menu[3].Sub_Title[1] = "ANGER_I";
	Menu[3].Sub_Type[1] = -4;
	Menu[3].Sub_Value[2] = &g_PID_DIR.D;
	Menu[3].Sub_Title[2] = "ANGER_D";
	Menu[3].Sub_Type[2] = -4;
	
	Menu[4].Title = "Huandao";
	Menu[4].Sub_Value[0] = &g_fHuandaoTurn;
	Menu[4].Sub_Title[0] = "Turn";
	Menu[4].Sub_Type[0] = -4;
	Menu[4].Sub_Value[1] = &g_nHuandaoTimer;
	Menu[4].Sub_Title[1] = "Timer";
	Menu[4].Sub_Type[1] = -2;
	Menu[4].Sub_Value[2] = &g_nHuandaoAnger;
	Menu[4].Sub_Title[2] = "Anger";
	Menu[4].Sub_Type[2] = -2;
	
	Menu[5].Title = "Podao";
	Menu[5].Sub_Value[0] = &g_nPodaoFlag;
	Menu[5].Sub_Title[0] = "Flag";
	Menu[5].Sub_Type[0] = -2;
	Menu[5].Sub_Value[1] = &g_nPodaoTimer;
	Menu[5].Sub_Title[1] = "Timer";
	Menu[5].Sub_Type[1] = -2;
	Menu[5].Sub_Value[2] = &g_nPodaoThres;
	Menu[5].Sub_Title[2] = "Thres";
	Menu[5].Sub_Type[2] = -2;
	Menu[5].Sub_Value[3] = &g_nPodaoSpeed;
	Menu[5].Sub_Title[3] = "Speed";
	Menu[5].Sub_Type[3] = -2;
}
...
static void Menu_HomePage_Show(void)
{
	...
	for(temp = 0; temp < MENU_MAX; ++temp)
	{
		...
		{
			OLED_P6x8Str(MENU_SHOW((temp % 2)) + 8, temp / 2 + 1, (char*)Menu[temp].Title);
		}
	}
}
...
static void Menu_SubPage_Show(Menu_TypeDef* Menu_Type)
{
	...
	OLED_P6x8Str((OLED_X_MAX - 1 - 6 * temp) / 2, 0, (char*)Menu_Type->Title);
	
	for(temp = 0; temp < Menu_Type->Sub_Value_Num; ++temp)
	{
		OLED_P6x8Str(8, temp + 1, (char*)Menu_Type->Sub_Title[temp]);
		...
		else if(Menu_Type->Sub_Type[temp] == -4)
		{
			if(*((float*)Menu_Type->Sub_Value[temp]) >= 0)
				OLED_P6x8Flo(80, temp + 1, *((float*)Menu_Type->Sub_Value[temp]) + 0.0001, -2);
			else
				OLED_P6x8Flo(80, temp + 1, *((float*)Menu_Type->Sub_Value[temp]) - 0.0001, -2);
		}
	}
	
	OLED_P6x8Str(8, 6, "SAVE");
	OLED_P6x8Str(8, 7, "BACK");
}
...
void Menu_Init(void)
{
	...
	for(temp = 0; temp < MENU_MAX; ++temp)
	{
		for(Sub_temp = 0; (Menu[temp].Title != 0) && ((Sub_temp < SUBMENU_MAX) && (Menu[temp].Sub_Title[Sub_temp] != 0)); ++Sub_temp)
		{
			...
			{
				if(*((int16*)Copy_Data[Sub_Menu_Sum & 0x0f]) != flash_read(FLASH_SAVE_RAW_MENU, 32 * Sub_Menu_Sum, int16))
					Sub_Menu_Sum |= 0x10;
			}
			else if(Menu[temp].Sub_Type[Sub_temp] == 2)
			{
				if(*((uint16*)Copy_Data[Sub_Menu_Sum & 0x0f]) != flash_read(FLASH_SAVE_RAW_MENU, 32 * Sub_Menu_Sum, uint16))
					Sub_Menu_Sum |= 0x10;
			}
			else if(Menu[temp].Sub_Type[Sub_temp] == -4)
			{
				if(*((float*)Copy_Data[Sub_Menu_Sum & 0x0f]) != flash_read(FLASH_SAVE_RAW_MENU, 32 * Sub_Menu_Sum, float))
					Sub_Menu_Sum |= 0x10;
			}
			++Sub_Menu_Sum;
		}
	...
	if(Sub_Menu_Sum & 0x10)//如果上面的结果为不一样，则说明Menu里面的变量值存在修改，则将之前FLASH中的参数重新写入，以备下一次检查用
	{
		...
		for(temp = 0; temp < Sub_Menu_Sum; ++temp)
		{
			if(Copy_Type[temp] == -2)
			{
				FLASH_WriteSector(FLASH_SAVE_RAW_MENU, 32 * temp, *((int16*)Copy_Data[temp]));
				FLASH_WriteSector(FLASH_SAVE_PRO_MENU, 32 * temp, *((int16*)Copy_Data[temp]));
			}
			else if(Copy_Type[temp] == 2)
			{
				FLASH_WriteSector(FLASH_SAVE_RAW_MENU, 32 * temp, *((uint16*)Copy_Data[temp]));
				FLASH_WriteSector(FLASH_SAVE_PRO_MENU, 32 * temp, *((uint16*)Copy_Data[temp]));
			}
			else if(Copy_Type[temp] == -4)
			{
				FLASH_WriteSector(FLASH_SAVE_RAW_MENU, 32 * temp, *((int32*)Copy_Data[temp]));
				FLASH_WriteSector(FLASH_SAVE_PRO_MENU, 32 * temp, *((int32*)Copy_Data[temp]));
			}
		}
	}
		...
		for(temp = 0, Sub_Sum_temp = 0; temp < MENU_MAX; ++temp)
		{
			if(Menu[temp].Title != 0)
			{
				for(Sub_temp = 0; Sub_temp < Menu[temp].Sub_Value_Num; ++Sub_temp)
				{
					if(Menu[temp].Sub_Type[Sub_temp] == -2)
					{
						*((int16*)Copy_Data[Sub_Sum_temp]) = flash_read(FLASH_SAVE_PRO_MENU, 32 * Sub_Sum_temp, int16);
					}
					else if(Menu[temp].Sub_Type[Sub_temp] == 2)
					{
						*((uint16*)Copy_Data[Sub_Sum_temp]) = flash_read(FLASH_SAVE_PRO_MENU, 32 * Sub_Sum_temp, uint16);
					}
					else if(Menu[temp].Sub_Type[Sub_temp] == -4)
					{
						*((float*)Copy_Data[Sub_Sum_temp]) = flash_read(FLASH_SAVE_PRO_MENU, 32 * Sub_Sum_temp, float);
					}
					++Sub_Sum_temp;
				}
			}
...
uint8 Menu_Work(void)
{
		...
		else//一级菜单确认标志置一，此时OLED显示为二级菜单
		{
			...
			else//否则，按下左边则为调整变量的值
			{
				if(Menu[Current_Count].Sub_Type[Sub_Current_Count] == -2)
				{
					...
					OLED_P6x8Int(80, Sub_Current_Count + 1, temp_Data[Sub_Current_Count], -3);
				}
				...
				{
					if(Button_Flag & 0x01) temp_Data[Sub_Current_Count] += 0.1;
					if(Button_Flag & 0x02) temp_Data[Sub_Current_Count] -= 0.1;
					if(Button_Flag & 0x04) temp_Data[Sub_Current_Count] -= 0.01;
					if(Button_Flag & 0x08) temp_Data[Sub_Current_Count] += 0.01;
					if(temp_Data[Sub_Current_Count] >= 0)
						OLED_P6x8Flo(80, Sub_Current_Count + 1, temp_Data[Sub_Current_Count] + 0.0001, -2);
					else
						OLED_P6x8Flo(80, Sub_Current_Count + 1, temp_Data[Sub_Current_Count] - 0.0001, -2);
				}
			}
	...
	{
		OLED_P6x8Str(MENU_SHOW(Current_Count_Last % 2), (Current_Count_Last) / 2 + 1, " ");
		OLED_P6x8Str(MENU_SHOW(Current_Count % 2), (Current_Count) / 2 + 1, ">");
	}
	else
	{
		OLED_P6x8Str(0, Sub_Current_Count_Last + 1, " ");
		if(Sub_Current_Count < Menu[Current_Count].Sub_Value_Num && Sub_Confirm_Flag == 1)
			OLED_P6x8Str(0, Sub_Current_Count + 1, "*");
		else OLED_P6x8Str(0, Sub_Current_Count + 1, ">");
	}
						...
						for(temp = 0; temp < Menu[Current_Count].Sub_Value_Num; ++temp)//将temp_Data中的临时储存值保存到变量
							{
								...
								{
									if(temp_Data[temp] >= 0)
										*((int16*)Copy_Data[temp + Copy_Data_Cnt]) = (int16)(temp_Data[temp] + 0.0001);
									else
										*((int16*)Copy_Data[temp + Copy_Data_Cnt]) = (int16)(temp_Data[temp] - 0.0001);
								}
								else if(Menu[Current_Count].Sub_Type[temp] == 2)
								{
									*((uint16*)Copy_Data[temp + Copy_Data_Cnt]) = (uint16)(temp_Data[temp] + 0.0001);
								}
								else if(Menu[Current_Count].Sub_Type[temp] == -4)
								{
									if(temp_Data[temp] >= 0)
										*((float*)Copy_Data[temp + Copy_Data_Cnt]) = (int16)((temp_Data[temp] + 0.0001) * 100)  / 100.0;//最大程度的保证float的精度
									else
										*((float*)Copy_Data[temp + Copy_Data_Cnt]) = (int16)((temp_Data[temp] - 0.0001) * 100)  / 100.0;//最大程度的保证float的精度
								}
							}
	...
	if(Button_Flag != 0) gpio_set(BUZZER, 1);

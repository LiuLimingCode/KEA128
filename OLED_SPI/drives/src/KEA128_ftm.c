...
//***********************************************************************************
//  @brief      输入捕获初始化  基于输入捕捉功能的计算编码器速度，用于节能组测速
//  @param      ftm             选择模块
//              ch              选择ftm通道
//  @by         llm
//***********************************************************************************
void ftm_period_measure_init(FTMn ftmn, FTM_CHn ch)
{
	ftm_pwm_mux(ftmn, ch);
	
	FTMX[ftmn]->SC = ( 0
                       //| FTM_SC_CPWMS_MASK       //0：边沿对齐 ，1： 中心对齐 （注释了表示 0）
                       | FTM_SC_PS(2)             //分频因子，分频系数 = 2^PS
                       | FTM_SC_CLKS(1)            //时钟选择， 0：没选择时钟，禁用； 1：系统时钟； 2：固定时钟； 3：外部时钟
                       //| FTM_SC_TOIE_MASK        //溢出中断使能（注释了表示 禁止溢出中断）
                     );

	
	FTMX[ftmn]->MOD = 0xffff;
...
//***********************************************************************************
//  @brief      获得转速        在速度较大的时候（大于200的时候）会出现明显误差
//  @param      ftm             选择模块
//              ch              选择ftm通道
//              times           该函数的调用周期
//  @by         llm
//***********************************************************************************
uint16 ftm_period_measure_get(FTMn ftmn, FTM_CHn ch, uint16 time)
{
	uint16 temp, flag = 0;
	FTMX[ftmn]->CONTROLS[ch].CnSC &= ~ FTM_CnSC_CHF_MASK;
	while(!(FTMX[ftmn]->CONTROLS[ch].CnSC & FTM_CnSC_CHF_MASK));
	FTMX[ftmn]->CNT = 0;
	FTMX[ftmn]->CONTROLS[ch].CnV = 0;
	FTMX[ftmn]->SC &= ~ FTM_SC_TOF_MASK;
	FTMX[ftmn]->CONTROLS[ch].CnSC &= ~ FTM_CnSC_CHF_MASK;
	while(!(FTMX[ftmn]->CONTROLS[ch].CnSC & FTM_CnSC_CHF_MASK));
	temp = FTMX[ftmn]->CONTROLS[ch].CnV;
	(FTMX[ftmn]->SC & FTM_SC_TOF_MASK)? (flag = 1): (flag = 0);
	temp = 10000000.0 / ((float)(0xffff * flag + temp)) / (ftm_clk_khz / 40000.0) / (1000.0 / time);

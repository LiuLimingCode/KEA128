...
#define SW3             B1
#define SW2             B1 
#define AD_CH_L_1       ADC0_SE0
#define AD_CH_L__       ADC0_SE0
#define AD_CH_M_1       ADC0_SE0
#define AD_CH_R__       ADC0_SE0
#define AD_CH_R_1       ADC0_SE0
...
int main()
{
  ...
  while(1)
  {
    OLED_P6x8Int(OLED_SHOW(1), 3, adc_read(AD_CH_L_1, 1), 3);
    OLED_P6x8Int(OLED_SHOW(2), 3, adc_read(AD_CH_L__, 1), 3);
    OLED_P6x8Int(OLED_SHOW(3), 3, adc_read(AD_CH_M_1, 1), 3);
    OLED_P6x8Int(OLED_SHOW(4), 3, adc_read(AD_CH_R__, 1), 3);
    OLED_P6x8Int(OLED_SHOW(5), 3, adc_read(AD_CH_R_1, 1), 3);//显示电感读数
    systick_delay_ms(10);
  }
}
...
void Normalized_MCP41(void)
{
	...
	while(1)
	{
		...
		if(gpio_get(KC) == 0)//如果按下确认键，那么开始调整电阻值
		{
			...
			systick_delay_ms(50);
			ADC_MinData[_index - 1] = adc_read(MCP41_ADC[_index - 1], 16);//保存此时的最小值
